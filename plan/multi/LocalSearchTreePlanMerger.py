...
class LocalSearchTreePlanMerger:
    ...
    You can choose between two local search heuristics: Tabu-L and Simulated Annealing by give the correct parameters
    to runTest function (see Examples in test).
    While passing parameters (see Examples in test) user can set the time limit of the local search, and to choose if the
    developing of new neighbors will be deterministic(like we do for testing) or will be done randomly.
    ...
    def __init__(self, patterns: Pattern or List[Pattern],
        ...
        self.patterns = patterns
        # we use that mapping in fixing SeqOperator events and to update pattern statistics while developing new neighbors.
        self.original_tree_plan_event_name_to_event_index_mapping_list = \
            self.set_original_tree_plan_event_name_to_event_index_mapping_list(pattern_to_tree_plan_map)
        ...
        self.is_neighbor_developing_deterministic = local_search_params[2]

    def set_original_tree_plan_event_name_to_event_index_mapping_list(self, pattern_to_tree_plan_map):
        """
        a list such that in place i will be a dictionary for pattern i in patterns.
        The dictionary is between event name in pattern to its original index in the original pattern (and also in its tree plan).
        """
        return [
    ...
    def find_mcs_to_patterns_that_dont_share_it_in_solution_mapping(self, mcs_to_pattern_dict):
        ...
        for mcs, patterns_mcs in self.mpg.mcs_to_patterns.items():
            if mcs not in mcs_to_pattern_dict.keys():
                difference_dict[mcs] = copy.deepcopy(self.mpg.mcs_to_patterns[mcs])
            else:
                patterns_difference = copy.deepcopy(self.mpg.mcs_to_patterns[mcs]) - mcs_to_pattern_dict[mcs]
    ...
    def neighbourhood(self, cur_sol:Solution):
        ...
        left_possible_shares_dict = self.find_mcs_to_patterns_that_dont_share_it_in_solution_mapping(copy.deepcopy(cur_sol.mcs_to_patterns_sharing))
        while left_possible_shares_dict and num_of_developed_neighbours < MAX_NEIGHBOURS_TO_DEVELOP:
            if self.is_neighbor_developing_deterministic:
                mcs, patterns_indices = list(left_possible_shares_dict.items())[0]
            else:
